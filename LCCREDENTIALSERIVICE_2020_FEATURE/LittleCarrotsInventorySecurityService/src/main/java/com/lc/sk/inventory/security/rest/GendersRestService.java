...
public class GendersRestService {
	@Autowired
	...
	public List<Genders> getAllGendersDetails() {
		key = headerKeyManagement.getKey(ConstantValues.INVENTORY_API_ACCESS_KEY);
		urls = serviceUrlsDao.getAccessUrl(UrlDetails.GENDERS_GET);
		List<Genders> genders = null;
			if (!urls.getUrl().equals(NextServiceURLMapping.ERROR_URL)) {
				...
				if (response.getStatusCode() == HttpStatus.ACCEPTED) {
					Genders[] gender = (Genders[]) JsonToBeanConverter.convert(response.getBody(), Genders[].class);
					genders = ArrayToListConverter.convertArrayToList(gender);
				}
	...
	public Genders getGenderById(String genderid) {
		key = headerKeyManagement.getKey(ConstantValues.INVENTORY_API_ACCESS_KEY);
		urls = serviceUrlsDao.getAccessUrl(UrlDetails.GENDERS_GET);
		Genders genders = null;
			if (!urls.getUrl().equals(NextServiceURLMapping.ERROR_URL)) {
				...
				response = restTemplate.exchange(urls.getUrl(), urls.getMethod(), request, String.class);
				if (response.getStatusCode() == HttpStatus.ACCEPTED) {
					genders = (Genders) JsonToBeanConverter.convert(response.getBody(), Genders.class);
				}
				else if(response.getStatusCode()==HttpStatus.OK)
				{
					ResponseBean resBean = (ResponseBean) JsonToBeanConverter.convert(response.getBody(),
